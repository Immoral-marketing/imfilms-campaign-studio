...
const MediaPlanSimpleView = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setFeedback(campaignData.media_plan_simple_feedback || "");
    ...
    const handleApprove = async () => {
        try {
            const { error } = await supabase
                .from('campaigns')
                .update({
                    media_plan_simple_status: 'aprobado',
                    media_plan_simple_feedback: null
                })
                .eq('id', id);
            ...
            try {
                await supabase.functions.invoke('send-email', {
                    body: {
                        type: 'media_plan_simple_approved',
                        campaignId: id,
    ...
    const handleReject = async () => {
        ...
        try {
            const { error } = await supabase
                .from('campaigns')
                .update({
                    media_plan_simple_status: 'rechazado',
                    media_plan_simple_feedback: feedback
                })
                .eq('id', id);
            ...
            try {
                await supabase.functions.invoke('send-email', {
                    body: {
                        type: 'media_plan_simple_rejected',
                        campaignId: id,
                        campaignTitle: campaign?.films?.title || 'Campaña',
                        distributorName: campaign?.distributors?.company_name || 'Distribuidora'
                    }
                ...
                <Card className="cinema-card border-cinema-gold/30 overflow-hidden">
                    <div className={`p-6 border-b border-cinema-gold/20 flex items-center justify-between ${status === 'aprobado' ? 'bg-green-500/10' :
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-sm uppercase tracking-wider font-bold text-cinema-ivory/60">Estado del Plan:</span>
                    ...
                    <div className="p-8 space-y-8">
                        <div>
                            <h3 className="font-cinema text-2xl text-cinema-gold mb-2 uppercase">Estrategia Detallada</h3>
                            <p className="text-cinema-ivory/80 mb-6">
                                Haz clic en el botón a continuación para abrir el documento externo con el desglose del plan de medios.
                            </p>

                            {campaign?.media_plan_simple_url ? (
                                <Button
                                    size="lg"
                                    ...
                                    onClick={() => window.open(campaign.media_plan_simple_url, '_blank')}
                                >
                                    <ExternalLink className="w-5 h-5 mr-3" />
                                    Abrir Plan de Medios
                                </Button>
                            ) : (
                                <p className="text-red-400 italic">No se ha proporcionado un enlace para este plan.</p>
                            )}
                        </div>
                        ...
                        {status === 'rechazado' && !showRejectForm && (
                            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
                                ...
                                <p className="text-cinema-ivory/80 italic text-sm">"{campaign.media_plan_simple_feedback}"</p>
                            </div>
                        )}
                    </div>
                </Card>
...
export default MediaPlanSimpleView;
