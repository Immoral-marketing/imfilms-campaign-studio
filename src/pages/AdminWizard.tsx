...
import { Clapperboard, Globe, Mail, Info, RotateCcw, CalendarClock, CalendarRange, CalendarCheck, FileText, ArrowLeft, Check, Loader2, Eye, EyeOff, Euro, Percent, Zap, Trash2, Plus, Upload, ExternalLink, Settings2, Download, Copy } from "lucide-react";
...
const AdminWizard = () => {
  ...
  // Admin wizard: success state with magic link
  const [adminSubmitted, setAdminSubmitted] = useState(false);
  const [adminMagicLink, setAdminMagicLink] = useState<string | null>(null);
  ...
  // No auth guard: admin-wizard is accessible directly by URL
  ...
  const isFirstRelease = true; // Admin always creates new accounts
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      // 1. Create user via admin API (no OTP needed, no session required)
      const { data: createData, error: createError } = await supabase.functions.invoke('admin-create-user', {
        body: {
          ...
          contactPhone: `${countryCode} ${signupData.contactPhone}`,
          tempPassword: signupData.password,
          adminSecret: import.meta.env.VITE_ADMIN_WIZARD_SECRET,
        },
      });
      ...
      const newUserId: string = createData.userId;
      ...
      const campaignPayload = {
        ...
        addons: Object.entries(selectedAddons).filter(([_, v]) => v).map(([k]) => k),
        contactData: { ...signupData, contactPhone: `${countryCode} ${signupData.contactPhone}` },
        notifyAdmin: false,
      };

      const { data: submitData, error: submitError } = await supabase.functions.invoke('submit-campaign', {
        body: campaignPayload,
      });

      if (submitError) throw new Error("Error al guardar la campaña.");
      const campaignId: string | undefined = submitData?.campaignId;

      // Notify distributor only (no admin notification from admin-wizard)
      try {
        ...
        console.error("Failed to send distributor notification email:", emailError);
      }

      toast.success("¡Cuenta y campaña creadas correctamente!");
      setAdminMagicLink(createData.magicLink || null);
  ...
  return (
    <>
                    ...
                    <div className="mt-8 space-y-8 animate-in fade-in duration-500">
                      ...
                      {parseFloat(adInvestment) > 0 && (
                        <div className="animate-in fade-in duration-500">
                          <CostSummary costs={costs} isFirstRelease={isFirstRelease} compact showPrices={isDistributor} feeMode={feeMode} skipOtp={true} />
                        </div>
                      )}
                    </div>
                  )}
      ...
      <GlobalHelpButton context="wizard" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <Card className="cinema-card w-full max-w-lg p-4 md:p-8 space-y-6">
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <h2 className="font-cinema text-3xl text-primary">Cuenta creada</h2>
              <p className="text-muted-foreground">
                La cuenta de <strong className="text-cinema-ivory">{signupData.companyName}</strong> ha sido creada y la campaña está enviada.
              </p>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="border-cinema-yellow text-cinema-yellow hover:bg-cinema-yellow/10 shrink-0"
                    >
                      {adminLinkCopied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                    </Button>
                  </div>
                </div>
              )}

              <div className="bg-cinema-charcoal/40 border border-border rounded-lg p-4 space-y-2">
                <p className="text-sm font-semibold text-cinema-ivory">Acceso temporal</p>
                <div className="text-sm space-y-1">
                  <div><span className="text-muted-foreground">Email:</span><span className="ml-2 text-cinema-ivory">{signupData.contactEmail}</span></div>
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground">Contraseña:</span>
                    <span className="ml-2 text-cinema-ivory font-mono">{signupData.password}</span>
                    <Button size="sm" variant="ghost" className="h-6 px-2" onClick={() => { navigator.clipboard.writeText(signupData.password); setAdminPasswordCopied(true); setTimeout(() => setAdminPasswordCopied(false), 2000); }}>
                      {adminPasswordCopied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-3 justify-center">
              <Button onClick={() => navigate("/campaigns")} className="bg-cinema-yellow text-black hover:bg-cinema-yellow/90">
                Volver a campañas
              </Button>
              <Button variant="outline" className="border-border" onClick={() => { setAdminSubmitted(false); setAdminMagicLink(null); setCurrentStep(1); setSignupData({ companyName: "", contactName: "", contactEmail: "", contactPhone: "", password: "", comments: "" }); }}>
                Crear otra cuenta
              </Button>
            </div>
          </Card>
